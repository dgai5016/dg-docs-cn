...
dg-docs-cn 的索引页（`site-index/`）是一个独立的 VitePress 项目，自动从仓库根的所有 `.project.json` 文件中读取项目列表。

## 工作流程

```
{repo-root}/
├── dataview/.project.json ─────┐
├── nextjs/.project.json ───────┤
├── vite/.project.json ─────────┤── build-projects.mjs 扫描
│                               │
│                               ▼
...
`.project.json` 字段到卡片展示的映射：

| .project.json 字段 | 卡片展示 |
|-------------------|---------|
| `title` | 卡片主标题（`<h3>`） |
| `description` | 卡片描述文字（最多 2 行） |
| `framework` | 左上角徽章（mkdocs 紫色，vitepress 绿色） |
...
| 卡片显示但点击 404 | 项目目录下框架配置有问题，CI 跳过了构建；看 Actions 日志 |
| 卡片描述显示「暂无描述」 | `.project.json` 的 `description` 字段为空，补上即可 |
| 框架徽章颜色错 | 检查 `.project.json` 的 `framework` 字段拼写（必须小写：`mkdocs` / `vitepress`） |
