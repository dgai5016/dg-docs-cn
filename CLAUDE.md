...
遇到其他框架（Docusaurus、Docsify 等）不要现场适配，直接报错退出，要求先在通用 skill 的 references 里加 framework 适配器。
...
编排 skill 会：调用通用 `dg-translate-tech-docs` 翻译全文 → 调用 `dg-import-docs` 搬运到 `dg-docs-cn/{项目名}/` → 配好 base URL → 生成 `.project.json` → 启动本地预览。
...
| Skill | 位置 | 通用性 |
|-------|------|--------|
| `dg-translate-tech-docs` | 外部 `dg-skills` 仓库 | ✅ 通用，可发布 |
| `dg-import-docs` | `.claude/skills/` | ❌ 绑定本仓库目录结构 |
| `dg-translate-and-import` | `.claude/skills/` | ❌ 绑定本仓库目录结构 |

通用 skill（翻译引擎、术语一致性）放在外部 `dg-skills` 仓库；与本仓库目录结构强耦合的（搬运、编排）放在本仓库 `.claude/skills/`。新增 skill 时遵循同样的"通用 vs 专属"分离原则。
